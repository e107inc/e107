<?php
/*
+---------------------------------------------------------------+
| Fichiers de langage Français e107 CMS (utf-8). Licence GNU/GPL
| Traducteurs: communauté francophone e107
|     $Source: /cvs_backup/e107_langpacks/e107_languages/French/admin/lan_filemanager.php,v $
|     $Revision: 1.4 $
|     $Date: 2006-12-10 08:43:18 $
|     $Author: daddycool78 $
+---------------------------------------------------------------+
*/
define("FMLAN_1", "Uploadé");
define("FMLAN_2", "dans le");
define("FMLAN_3", "répertoire");
define("FMLAN_4", "Le fichier uploadé excède la taille maximale autorisée par la variable upload_max_filesize dans php.ini.");
//define("FMLAN_5", "Le fichier uploadé excède la taille maximale qui est spécifié dans le formulaire HTML par la variable MAX_FILE_SIZE.");
//define("FMLAN_6", "Le fichier uploadé n'a été que partiellement uploadé.");
//define("FMLAN_7", "Aucun fichier n'a été uploadé.");
//define("FMLAN_8", "Le fichier uploadé fait 0 octet");
//define("FMLAN_9", "Le fichier n'a pas été uploadé. Nom du fichier");
//define("FMLAN_10", "Erreur");
//define("FMLAN_11", "Les permissions du répertoire d'upload sont probablement incorrectes.");
define("FMLAN_12", "fichier");
define("FMLAN_13", "fichiers");
define("FMLAN_14", "répertoire");
define("FMLAN_15", "répertoires");
define("FMLAN_16", "Répertoire racine");
define("FMLAN_17", "Nom");
define("FMLAN_18", "Taille");
define("FMLAN_19", "Dernière modification");
define("FMLAN_21", "Uploader un fichier dans ce répertoire");
define("FMLAN_22", "Uploader");
define("FMLAN_26", ""); // "Supprimé"
define("FMLAN_27", "supprimé avec succès"); // "avec succès"
define("FMLAN_28", "Impossible de supprimer");
define("FMLAN_29", "Chemin");
define("FMLAN_30", "Revenir au répertoire précédent");
define("FMLAN_31", "dossier");
define("FMLAN_32", "Sélectionner un répertoire");
define("FMLAN_33", "Sélectionner");
define("FMLAN_34", "Choix du répertoire");
define("FMLAN_35", "Répertoire des fichiers");
define("FMLAN_36", "Répertoire des menus personnalisables");
define("FMLAN_37", "Répertoire des pages personnalisables");
define("FMLAN_38", "Fichier déplacé avec succès vers ");
define("FMLAN_39", "Impossible de déplacer le fichier vers");
define("FMLAN_40", "Répertoire des Images-".GLOBAL_LAN_NEWS_4."");
define("FMLAN_41", "Déplacer vers le répertoire Téléchargements.");
define("FMLAN_42", "Déplacer vers le répertoire Images-Téléchargements.");
define("FMLAN_43", "Supprimer les fichiers sélectionnés");
define("FMLAN_44", "Veuillez confirmer que vous souhaitez déplacer les fichiers sélectionnés vers le répertoire Téléchargements.");
define("FMLAN_45", "Veuillez confirmer que vous souhaitez déplacer les fichiers sélectionnés vers le répertoire Images-Téléchargements.");
define("FMLAN_46", "Veuillez confirmer que vous souhaitez supprimer les fichiers sélectionnés.");
define("FMLAN_47", "Uploads des utilisateurs");
define("FMLAN_48", "Déplacer la sélection vers");
define("FMLAN_49", "Veuillez confirmer que vous souhaitez déplacer les fichiers sélectionnés.");
define("FMLAN_50", "Déplacer");
?>
