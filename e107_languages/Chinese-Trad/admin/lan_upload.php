<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 網站 system - Language File.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Chinese-Trad/admin/lan_upload.php,v $
|     $Revision: 1.1 $
|     $Date: 2006-09-16 15:12:33 $
|     $Author: e107coders $
+----------------------------------------------------------------------------+
*/
define("UPLLAN_1", "Upload 已移除的 from list.");
define("UPLLAN_2", "設定 已儲存的 in 資料庫");
define("UPLLAN_3", "Upload ID");
// define("UPLLAN_4", "Date");
define("UPLLAN_5", "發表人");
define("UPLLAN_6", "Email");
define("UPLLAN_7", "網站");
define("UPLLAN_8", "File Name");

define("UPLLAN_9", "Version");
define("UPLLAN_10", "File");
define("UPLLAN_11", "File Size");
define("UPLLAN_12", "Screenshot");
define("UPLLAN_13", "Description");
define("UPLLAN_14", "Demo");
// define("UPLLAN_15", "Options");
define("UPLLAN_16", "copy to newspost");
define("UPLLAN_17", "remove upload from list");
define("UPLLAN_18", "View details");
define("UPLLAN_19", "There are no unmoderated public 上傳");
define("UPLLAN_20", "There");
define("UPLLAN_21", "unmoderated public upload");
define("UPLLAN_22", "ID");
define("UPLLAN_23", "Name");
define("UPLLAN_24", "Filetype");
define("UPLLAN_25", "上傳已開啟?");
define("UPLLAN_26", "No public 上傳 will be permitted if disabled");
// define("UPLLAN_27", "Yes");
// define("UPLLAN_28", "No");

define("UPLLAN_29", "Storage type");
define("UPLLAN_30", "Choose how to store uploaded files, either as normal files on server or as binary 資訊 in 資料庫<br /><b>Note</b> binary is only suitable for smaller files under approximately 500kb");
define("UPLLAN_31", "Flatfile");
define("UPLLAN_32", "Binary");
define("UPLLAN_33", "Maximum file size");
define("UPLLAN_34", "Maximum upload size in bytes - leave blank to conform to php.ini setting ( php.ini setting is");
define("UPLLAN_35", "Allowed file types");
define("UPLLAN_36", "請 enter one type per line");
define("UPLLAN_37", "Permission");
define("UPLLAN_38", "Select to allow only certain 會員s to upload");
define("UPLLAN_39", "Submit");
// define("UPLLAN_40", "Options");
define("UPLLAN_41", "請 note - file 上傳 are disabled from your php.ini, it will not be possible to upload files until you set it to On.");

define("UPLLAN_42", "Actions");
define("UPLLAN_43", "上傳");
define("UPLLAN_44", "Upload");

define("UPLLAN_45", "Are you sure you want to 刪除 the 下列的 file...");
// define("UPLLAN_46", "刪除");
// define("UPLLAN_47", "取消");

define("UPLAN_COPYTODLM", "copy to download manager");
define("UPLAN_IS", "is ");
define("UPLAN_ARE", "are ");
define("UPLAN_COPYTODLS", "Copy to 下載");

?>