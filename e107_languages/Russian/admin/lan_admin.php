...
define("ADLAN_ERR_3", "Имеется один или более файлов в ваших каталогах загрузок на сервер, которые не находятся в списке разрешенных для загрузки на севрвер файлов. Они могли быть помещены атакующим лицом и если это так, должны быть удалены <b>немедленно</b>. Вы <b>не</b> должны открывать эти файлы поскольку они могут запустить злонамеренный код, который может содержаться в файле, это означает: не открывайте их вашим браузером.<br /><br />Если вы признаете, что эти файлы допустимые, это может быть ввиду последнего изменения допустимых типов файлов, тип файлов больше не находится среди допустимых и вам нужно добавить его заново (смотрите Загрузки на севрер). Не следует разрешать загружать на сервер .html, .txt, и т. д., поскольку атакующее лицо может загрузить на сервер файл этого типа, включающий вредоносный javascript. Вам следует также, конечено, не позволять загружать на сервер .php файлы или какие-либо другие запускаемые скрипты.<br /><br />Ниже следует список файлов, которые могут быть потенциально злонамеренными:");
