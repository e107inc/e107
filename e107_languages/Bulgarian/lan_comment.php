<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language File
|     Bulgarian Language Pack for e107 Version 0.7
|     Copyright © 2005 - Bulgarian e107
|     http://www.e107bg.org
|     Encoding: utf-8
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Bulgarian/lan_comment.php,v $
|     $Revision: 1.5 $
|     $Date: 2007-03-04 21:39:35 $
|     $Author: secretr $
+----------------------------------------------------------------------------+
*/

define("PAGE_NAME", "Коментари");

define("COMLAN_0", "[забранено от администратора]");
define("COMLAN_1", "Разреши");
define("COMLAN_2", "Забрани");
define("COMLAN_3", "Изтрий");
define("COMLAN_4", "Инфо");
define("COMLAN_5", "Коментари ...");
define("COMLAN_6", "Трябва да се логнете за да пуснете коментар на този сайт - ако не сте регистриран потребител кликнете");
define("COMLAN_7", "Администратор на сайта");
define("COMLAN_8", "Коментар");
define("COMLAN_9", "Пусни коментар");
define("COMLAN_10", "Администратор");
define("COMLAN_11", "Записването на коментара беше неуспешно - моля провбвайте отново, като премахнете всички нестандартни знаци.");
define("COMLAN_16", "Потребителско име: ");
define("COMLAN_99", "Коментари");
define("COMLAN_100", "Новини");
define("COMLAN_101", "Анкета");
define("COMLAN_102", "Отговор на: ");
define("COMLAN_103", "Статия");
define("COMLAN_104", "Рецензия");
define("COMLAN_105", "Съдържание");
define("COMLAN_145", "Регистриран на: ");
define("COMLAN_194", "Гост");
define("COMLAN_195", "Регистриран потребител");
define("COMLAN_310", "Невъзможно поместването на коментара - използвате регистрирано потребителско име. Ако това е вашето потребителско име моля логнете се и пробвайте отново.");
define("COMLAN_312", "Дублиране - записването е невъзможно.");
define("COMLAN_313", "Местоположение");
define("COMLAN_314", "редактирай коментара");

/*
define("COMCOMLAN_1", "тук");
define("COMCOMLAN_2", "за регистрация");
define("COMCOMLAN_3", "Грешка!");
define("COMCOMLAN_4", 'Относно');
define("COMCOMLAN_5", 'В отговор на:');
define("COMCOMLAN_6", 'Отговори');
define("COMCOMLAN_7", 'Рейтинг');
define("COMCOMLAN_8", 'Коментарите за забранени');
*/

define("COMLAN_315", "Проследяване");
define("COMLAN_316", "Няма проследяване за тази публикация.");
define("COMLAN_317", "Редактирай проследяване");

define("COMLAN_318", "Редакция на коментар");
define("COMLAN_319", "редактиран");
define("COMLAN_320", "Обновяване на коментара");
define("COMLAN_321", "тук");
define("COMLAN_322", "за регистрация");
define("COMLAN_323", "Грешка!");

define("COMLAN_324", 'Относно');
define("COMLAN_325", 'Ре:');
define("COMLAN_326", 'Отговори на това');
define("COMLAN_327", 'Рейтинг');
define("COMLAN_328", 'Коментарите са спрени');
define("COMLAN_329", 'Неоторизиран');
define("COMLAN_330", 'IP:');

define("COMLAN_TYPE_1", "новини");
define("COMLAN_TYPE_2", "файлове");
define("COMLAN_TYPE_3", "чзв");
define("COMLAN_TYPE_4", "анкета");
define("COMLAN_TYPE_5", "документи");
define("COMLAN_TYPE_6", "бъгове");
define("COMLAN_TYPE_7", "идеи");
define("COMLAN_TYPE_8", "потребителски профил");

?>