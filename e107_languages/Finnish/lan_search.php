<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language Tiedosto.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Finnish/lan_search.php,v $
|     $Revision: 1.4 $
|     $Date: 2006-02-03 19:32:47 $
|     $Kirjoittaja: sweetas $
+----------------------------------------------------------------------------+
*/

define("PAGE_NAME", "Etsi");

define("LAN_98", "Uutiset");
define("LAN_99", "Kommentit");
define("LAN_140", "Käyttäjät");
define("LAN_180", "Etsi");
define("LAN_192", "Kaikki aihepiirit");
define("LAN_193", "Tapahtuma kalenteri");
define("LAN_194", "Kaikki Aihepiirit");
define("LAN_195", "Etsintä");
define("LAN_196", "osumia");

define("LAN_197", "Tiedostojen lataus");
define("LAN_198", "Osumia ei löytynyt");
define("LAN_199", "Etsi:");
define("LAN_200", "Aiheista:");

define("LAN_201", "Ole ystävällinen ja tarkenna etsittävää merkkijonoa");
define("LAN_416", "Sinun tulee olla kirjautunut käyttäjäksi päästäksesi tälle sivulle");
define("LAN_417", "Etsittävä merkkijono pitää olla vähintää 3-merkkiä.");

define("LAN_418", "Muut sivut");

define("LAN_SEARCH_1", "Valitse kaikki");
define("LAN_SEARCH_2", "Poista valinta kaikista");
define("LAN_SEARCH_3", "Lähetetty ");
define("LAN_SEARCH_4", "Match found in news title");
define("LAN_SEARCH_5", "Match found in news text");
define("LAN_SEARCH_6", "Match found in extended news text");
define("LAN_SEARCH_7", "Lähettäjä ");
define("LAN_SEARCH_8", " on ");
define("LAN_SEARCH_9", "Nimetön");
define("LAN_SEARCH_10", "Siirry sivulle:");
define("LAN_SEARCH_11", "Löytyneitä");
define("LAN_SEARCH_12", " of ");
define("LAN_SEARCH_13", " kohteessa ");
define("LAN_SEARCH_14", "Aihepiiri:");
define("LAN_SEARCH_15", "Kirjoittaja:");
define("LAN_SEARCH_17", "Sorry, search is restricted to one search every ");
define("LAN_SEARCH_18", " seconds.");
define("LAN_SEARCH_19", "Etsi kohteesta:");
define("LAN_SEARCH_20", "Kirjoittajaisation Required");
define("LAN_SEARCH_21", "You are not authorised to view this page.");



define("LAN_SEARCH_22", "Kaikki alueet");
define("LAN_SEARCH_23", "Enhanced Query Form");
define("LAN_SEARCH_24", "Tulee sisältää sanat");
define("LAN_SEARCH_25", "Ei saa sisältää sanoja");
define("LAN_SEARCH_26", "Sisältää lauseen");
define("LAN_SEARCH_27", "Word(s) beginning with");
define("LAN_SEARCH_28", "All Has No Advanced Etsi");
define("LAN_SEARCH_29", "Yksinkertainen");
define("LAN_SEARCH_30", "Monipuolinen");
define("LAN_SEARCH_31", "Has No Advanced Etsi");
define("LAN_SEARCH_32", "The following words were excluded from the search");
define("LAN_SEARCH_33", "The following word was excluded from the search");
define("LAN_SEARCH_34", "Uusier than");
define("LAN_SEARCH_35", "Older than");
define("LAN_SEARCH_36", "Any time");
define("LAN_SEARCH_37", "One päivä");
define("LAN_SEARCH_38", "Two päivää");
define("LAN_SEARCH_39", "Three päivää");
define("LAN_SEARCH_40", "One week");
define("LAN_SEARCH_41", "Two weeks");
define("LAN_SEARCH_42", "Three weeks");
define("LAN_SEARCH_43", "One month");
define("LAN_SEARCH_44", "Two months");
define("LAN_SEARCH_45", "Three months");
define("LAN_SEARCH_46", "Half a year");
define("LAN_SEARCH_47", "One year");
define("LAN_SEARCH_48", "Two years");
define("LAN_SEARCH_49", "Three Years");

define("LAN_SEARCH_50", "Date posted");
define("LAN_SEARCH_51", "Kaikki aihepiirit");
define("LAN_SEARCH_52", "Match in");
define("LAN_SEARCH_53", "Whole item");
define("LAN_SEARCH_54", "Otsikko only");
define("LAN_SEARCH_55", "Etsi in news category");
define("LAN_SEARCH_56", "All Uutiset Aihepiirit");
define("LAN_SEARCH_57", "Kommentteja to");
define("LAN_SEARCH_58", "Kaikki alueet");
define("LAN_SEARCH_59", "All Kommentit");
define("LAN_SEARCH_60", "Kommentteja to");
define("LAN_SEARCH_61", "By author");
define("LAN_SEARCH_62", "Date joined");
define("LAN_SEARCH_63", "Etsi in category");
define("LAN_SEARCH_64", "All Lataa Aihepiirit");
define("LAN_SEARCH_65", "Tiedostojen lataus");
define("LAN_SEARCH_66", "Lisätty");
define("LAN_SEARCH_67", "All downloads details");
define("LAN_SEARCH_68", "Päivä");
define("LAN_SEARCH_69", "Relevance");

define("LAN_SEARCH_70", "Lähetäed to download item");
define("LAN_SEARCH_71", "Lähetäed in vastaa to news item");
define("LAN_SEARCH_72", "Allekirjoitus");
define("LAN_SEARCH_73", "No Allekirjoitus.");
define("LAN_SEARCH_74", "Joined on");

define("LAN_SEARCH_75", "Etsi type");
define("LAN_SEARCH_76", "Lähetäed on page");
define("LAN_SEARCH_77", "Lähetäed on profile page of");

?>
