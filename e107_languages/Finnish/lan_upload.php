<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language Tiedosto.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Finnish/lan_upload.php,v $
|     $Revision: 1.2 $
|     $Date: 2006-01-14 22:26:04 $
|     $Kirjoittaja: e107coders $
+----------------------------------------------------------------------------+
*/
define("PAGE_NAME", "Lisää tiedosto");

define("LAN_20", "Virhe");
define("LAN_61", "Sinun nimesi: ");
define("LAN_112", "Sähköpostiosoite: ");
define("LAN_144", "www-sivu osoite: ");
define("LAN_402", "You must be a registered member to upload files to this server.");
define("LAN_403", "You do not have the correct permissions to upload files to this server.");
define("LAN_404", "Thank you. Your upload will be reviewed by an administrator ja posted to the site if appropriate.");
define("LAN_405", "Tiedosto exceeds specified maximum size limit - poistettu.");
define("LAN_406", "Please note");
define("LAN_407", "Any other filetypes uploaded will be instantly poistettu.");
define("LAN_408", "Underlined");
define("LAN_409", "Name of file");
define("LAN_410", "Versio");
define("LAN_411", "Tiedosto");
define("LAN_412", "Esikatselukuva");
define("LAN_413", "Kuvaus");
define("LAN_414", "Working demo");
define("LAN_415", "enter URL to site where demo can be viewed");
define("LAN_416", "Lähetä ja Lisää tiedosto");
define("LAN_417", "Lisää tiedosto Tiedosto");
define("LAN_418", "Maximum file size: ");
define("DOWLAN_11", "Aihepiiri");
define("LAN_419", "Kaikkiowed filetypes");
define("LAN_420", "fields are required");



?>
