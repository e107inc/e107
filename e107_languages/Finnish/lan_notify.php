<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language Tiedosto.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Finnish/lan_notify.php,v $
|     $Revision: 1.2 $
|     $Date: 2006-01-14 22:26:04 $
|     $Kirjoittaja: sweetas $
+----------------------------------------------------------------------------+
*/

define("NT_LAN_US_1", "User Signup");

define("NT_LAN_UV_1", "User Signup Verified");
define("NT_LAN_UV_2", "Käyttäjät session string");

define("NT_LAN_LI_1", "User Logged In");

define("NT_LAN_LO_1", "User Logged Out");
define("NT_LAN_LO_2", " logged out of site");

define("NT_LAN_FL_1", "Flood Ban");
define("NT_LAN_FL_2", "IP address banned for flooding");

define("NT_LAN_SN_1", "Uutiset Nimike Lähetäted");

define("NT_LAN_NU_1", "Päivitäd");

define("NT_LAN_ND_1", "Uutiset Nimike Poistad");
define("NT_LAN_ND_2", "Poistad news item id");

?>
