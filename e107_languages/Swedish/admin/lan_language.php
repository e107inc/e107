<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language File.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/Swedish/admin/lan_language.php,v $
|     $Revision: 1.4 $
|     $Date: 2007-01-18 06:38:28 $
|     $Author: mrpiercer $
+----------------------------------------------------------------------------+
*/
define("LANG_LAN_00", "kunde inte skapas. (Finns redan)");
define("LANG_LAN_01", "raderades (om befintlig) och skapades.");
define("LANG_LAN_02", "kunde inte raderas");
define("LANG_LAN_03", "Tabeller");
define("LANG_LAN_05", "Ej installerad");
define("LANG_LAN_06", "Skapa tabeller");
define("LANG_LAN_07", "Släppa befintliga tabeller?");
define("LANG_LAN_08", "Ersätta befintliga tabeller (data förloras).");
define("LANG_LAN_10", "Bekräfta radering");
define("LANG_LAN_11", "Radera omarkerade tabeller ovan (om de existerar).");
define("LANG_LAN_12", "Aktivera fler-språkstabeller");
define("LANG_LAN_13", "Flerspråkspreferenser");
define("LANG_LAN_14", "Sajtens standardspråk");
define("LANG_LAN_15", "Markera för att kopiera data från standardspråket.(användbart för länkar, nyhetskategorier etc) ");
define("LANG_LAN_16", "Flerspråksstöd i databasen.");
define("LANG_LAN_17", "Standardspråk - Inga övriga språktabeller är nödvändiga.");
define("LANG_LAN_18", "Använd subdomäner för att styra sajtens språkval");
define("LANG_LAN_19", "t ex no.minsajt.se för att presentera sajten på Norska. ");
define("LANG_LAN_20", "Ange sajtens domännamn, t ex mindomän.se");
define("LANG_LAN_21", "Språkverktyg");


?>