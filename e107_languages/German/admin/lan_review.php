<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language File.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/German/admin/lan_review.php,v $
|     $Revision: 1.3 $
|     $Date: 2005-07-26 14:43:22 $
|     $Author: gatowlion $
|     $translated by: admin@cms-myway.com (http://www.cms-myway.com) $
|     $ UTF-8 encoded $
+----------------------------------------------------------------------------+
*/
define("REVLAN_1", "Bericht in Datenbank eingefügt.");
define("REVLAN_2", "Felder sind freigelassen worden.");
define("REVLAN_3", "Bericht in Datenbank aktualisiert.");
define("REVLAN_4", "Bericht gelöscht.");
define("REVLAN_5", "Bitte markieren Sie die Bestätigungsbox um den Bericht zu löschen");
define("REVLAN_6", "Keine Berichte bis jetzt.");
define("REVLAN_7", "Bestehende Berichte");
define("REVLAN_8", "Bearbeiten");
define("REVLAN_9", "Löschen");
define("REVLAN_10", "markiere um zu bestätigen");
define("REVLAN_11", "Öffne den HTML Editor");
define("REVLAN_12", "Überschrift");
define("REVLAN_13", "Unter-Überschrift");
define("REVLAN_14", "Zusammenfassung");
define("REVLAN_15", "Bericht");
define("REVLAN_16", "Bewertung");
define("REVLAN_17", "Bitte wählen Sie eine Bewertung");
define("REVLAN_18", "Kommentare erlauben?");
define("REVLAN_19", "an");
define("REVLAN_20", "aus");
define("REVLAN_21", "Sichtbar für");
define("REVLAN_22", "Bericht aktualisieren");
define("REVLAN_23", "Bericht übermitteln");
define("REVLAN_24", "Berichte");




define("REVLAN_25", "Bericht Kategorie gespeichert");
define("REVLAN_26", "Bericht Kategorie aktualisiert");
define("REVLAN_27", "Kategorie gelöscht");
define("REVLAN_28", "Kategorie");
define("REVLAN_29", "Optionen");
define("REVLAN_30", "Bearbeiten");
define("REVLAN_31", "Löschen");
define("REVLAN_32", "Keine Bericht Kategorien");
define("REVLAN_33", "Bestehende Bericht Kategorien");
define("REVLAN_34", "Kategorien Name");
define("REVLAN_35", "Kategorien Icon");
define("REVLAN_36", "Bilder zeigen");
define("REVLAN_37", "Kategorien Übersicht");
define("REVLAN_38", "Bericht Kategorie updaten");
define("REVLAN_39", "Bericht Kategorie erstellen");
define("REVLAN_40", "Keine Berichte");
define("REVLAN_41", "Bestehende Berichte");
define("REVLAN_42", "Öffne den HTML Editor");
define("REVLAN_43", "Kategorie");
define("REVLAN_44", "Keine");
define("REVLAN_45", "Bericht erste Seite");
define("REVLAN_46", "Neuen Bericht erstellen");
define("REVLAN_47", "Kategorien");
define("REVLAN_48", "Bericht Optionen");
define("REVLAN_49", "Sind Sie sicher diese Kategorie löschen zu wollen?");
define("REVLAN_50", "Sind Sie sicher diesen Bericht löschen zu wollen?");
define("REVLAN_51", "Autor Angaben");
define("REVLAN_52", "lassen Sie das Feld frei, wenn Sie den Bericht geschrieben haben");
define("REVLAN_53", "Autor Name");
define("REVLAN_54", "Autor Email Adresse");


define("REVLAN_55", "Erlaube Berichte zu übermitteln");
define("REVLAN_56", "Erlaube Besuchern Berichte zu übermitteln");
define("REVLAN_57", "Übermittle Bericht Klasse");
define("REVLAN_58", "Wählen Sie welche User Berichte übermitteln können");
define("REVLAN_59", "Aktualisiere Optionen");
define("REVLAN_60", "Bericht Optionen");
define("REVLAN_61", "Bericht Optionen aktualisiert");

define("REVLAN_62", "Übermittelte Berichte");
define("REVLAN_63", "Keine Berichte übermittelt");
define("REVLAN_64", "Keine Email Adresse angegeben");
define("REVLAN_65", "Bericht Überschrift");
define("REVLAN_66", "Eintragen");
define("REVLAN_67", "User übermittelten Bericht eintragen");
define("REVLAN_68", "User übermittelter Bericht in Datenbank gespeichert");

define("REVLAN_69", "Formular zurücksetzen");
define("REVLAN_70", "Hier klicken um Autorfelder auszufüllen");
define("REVLAN_71", "E-Mail hinzufügen/Icons drucken?");
define("REVLAN_72", "Ja");
define("REVLAN_73", "Nein");






?>
