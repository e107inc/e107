<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system - Language File.
|
|     $Source: /cvs_backup/e107_langpacks/e107_languages/ChineseTrad/lan_comment.php,v $
|     $Revision: 1.2 $
|     $Date: 2007-09-09 07:18:26 $
|     $Author: e107coders $
+----------------------------------------------------------------------------+
*/

define("COMLAN_0", "[已經被管理員封鎖]");
define("COMLAN_1", "解除封鎖");
define("COMLAN_2", "封鎖");
define("COMLAN_3", "刪除");
define("COMLAN_4", "資訊");
define("COMLAN_5", "評論 ...");
define("COMLAN_6", "您必須登入以後才可以發表評論 - 請登入或是註冊成會員");
define("COMLAN_7", "核心網站管理員");
define("COMLAN_8", "評論");
define("COMLAN_9", "送出評論");
define("COMLAN_10", "管理員");
define("COMLAN_11", "無法輸入您的評論到資料庫中 - 請重新輸入並刪除非基本字元.");
define("COMLAN_16", "帳號: ");
define("COMLAN_99", "評論");
define("COMLAN_100", "新聞");
define("COMLAN_101", "投票");
define("COMLAN_102", "回覆給: ");
define("COMLAN_103", "文章");
define("COMLAN_104", "回顧");
define("COMLAN_105", "內容");
define("COMLAN_145", "已註冊: ");
define("COMLAN_194", "訪客");
define("COMLAN_195", "已註冊會員");
define("COMLAN_310", "無法接受已註冊會員名稱- 假如是您的帳號，請登入後發表.");
define("COMLAN_312", "複製發表 - 本站無法接受此行為.");
define("COMLAN_313", "來自於");
define("COMLAN_314", "編輯評論");
define("COMLAN_315", "引用回覆");
define("COMLAN_316", "該發表目前沒有引用回覆.");
define("COMLAN_317", "編輯引用回覆");
define("COMLAN_318", "編輯評論");
define("COMLAN_319", "編輯過");
define("COMLAN_320", "更新編輯");
/*
define("COMLAN_1", "這裡");
define("COMLAN_2", "登入");
define("COMLAN_3", "錯誤!");
define("COMLAN_4", '主題');
define("COMLAN_5", '回覆:');
define("COMLAN_6", '回覆該');
define("COMLAN_7", '評分');
define("COMLAN_8", '評論功能已關閉');
define("COMLAN_9", '尚未認可');
*/

define("COMLAN_321", "這裡");
define("COMLAN_322", "註冊");
define("COMLAN_323", "發生錯誤!");
define("COMLAN_324", '主題');
define("COMLAN_325", '回覆:');
define("COMLAN_326", '回覆給');
define("COMLAN_327", '評比中');
define("COMLAN_328", '評論功能關閉');
define("COMLAN_329", '尚未認可');
define("COMLAN_330", 'IP:');

define("COMLAN_TYPE_1", "新聞");
define("COMLAN_TYPE_2", "下載");
define("COMLAN_TYPE_3", "常見問題");
define("COMLAN_TYPE_4", "投票");
define("COMLAN_TYPE_5", "文件");
define("COMLAN_TYPE_6", "錯誤追蹤");
define("COMLAN_TYPE_7", "主意");
define("COMLAN_TYPE_8", "會員個人資料");

?>