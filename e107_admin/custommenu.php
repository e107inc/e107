...
if ((isset($_POST['add_menu']) || isset($_POST['update_menu'])) && $_POST['type_content'] == 1){
	...
	if (!$_POST['menu_name'] || !$_POST['menu_caption'] || !$_POST['menu_text']){
		...
		$_POST['menu_caption'] = stripslashes($_POST['menu_caption']);
		$_POST['menu_text'] = stripslashes($_POST['menu_text']);
 ...
 // Read Menu Information.
	...
	*/
	...
	if($fileArray = file($menu)){
		...
		$_POST['menu_text'] = str_replace($search, $replace, $match[1]);

		preg_match("/<CAPTION(.*?)CAPTION;/si", $data, $match);
		$_POST['menu_caption'] = str_replace($search, $replace, $match[1]);
		if($pref['wysiwyg']){
