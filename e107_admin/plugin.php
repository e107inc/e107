...
if(strstr(e_QUERY, "install")){
	...
	if($eplug_userclass){
		...
		if($i<255){
			$uclass_name = strip_tags(strtoupper($eplug_userclass));
			if(!$sql -> db_Count('userclass_classes','(*)',"WHERE userclass_name = '".$uclass_name."'"))
			{
				$sql -> db_Insert("userclass_classes", $i.", '".$uclass_name."', '{$eplug_userclass_description}' ,".e_UC_PUBLIC);
			}
		}
	}
