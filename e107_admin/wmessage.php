...
if (isset($_POST['wm_update'])) {
	...
	$message = ($sql->db_Update("wmessage", "wm_text ='$wm_text', wm_active='".$_POST['wm_active']."' WHERE wm_id='".$_POST['wm_id']."' ")) ? LAN_UPDATED :
	 LAN_UPDATED_FAILED;
}
	
if (isset($_POST['wm_insert'])) {
	$wmtext = $aj->toDB($_POST['wm_text']);
	$message = ($sql->db_Insert("wmessage", "0, '$wmtext', '".$_POST['wm_active']."' ")) ? LAN_CREATED :
	 LAN_CREATED_FAILED ;
}
...
if ($delete && $del_id) {
	$message = ($sql->db_Delete("wmessage", "wm_id='".$del_id."'  ")) ? LAN_DELETED :
	 LAN_DELETED_FAILED ;
}
