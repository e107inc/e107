...
if (isset($_POST['preview'])) {
	...
	$content_heading = $aj->formtpa($_POST['content_heading'], "admin");
	 $ch = $aj->tpa($content_heading, "nobreak", "admin");
	$content_subheading = $aj->formtpa($_POST['content_subheading'], "admin");
	 $cs = $aj->tpa($content_subheading, "nobreak", "admin");
	$dt = (strstr($_POST['data'], "[img]http") ? $_POST['data'] : str_replace("[img]", "[img]../", $_POST['data']));
	$dt = $aj->formtpa($dt, "admin");
	 $dt = $aj->tpa($dt, "off", "admin");
	$content_summary = $aj->formtpa($_POST['content_summary'], "admin");
	 $cu = $aj->tpa($content_summary, "nobreak", "admin");
...
if ($action == "cat") {
	...
	if ($id) {
		$text .= "<input class='button' type='submit' name='update_category' value='".ARLAN_69."' />
			".$rs->form_button("submit", "category_clear", "".ARLAN_70.""). $rs->form_hidden("category_id", $id)."
			</td></tr>";
	} else {
...
if ($action == "create") {
	...
	($content_comment ? ARLAN_22.": <input type='radio' name='content_comment' value='1' checked='checked' />".ARLAN_23.": <input type='radio' name='content_comment' value='0' />" : ARLAN_22.": <input type='radio' name='content_comment' value='1' />".ARLAN_23.": <input type='radio' name='content_comment' value='0' checked='checked' />")."
		...
		<tr>
		<td colspan='2' class='forumheader3'>".ARLAN_24.":&nbsp;&nbsp;". ($content_pe_icon ? ARLAN_25.": <input type='radio' name='add_icons' value='1' checked='checked' />".ARLAN_26.": <input type='radio' name='add_icons' value='0' />" : ARLAN_25.": <input type='radio' name='add_icons' value='1' />".ARLAN_26.": <input type='radio' name='add_icons' value='0' checked='checked' />")."
		</td>
		</tr>
		...
		<td colspan='2'  style='text-align:center' class='forumheader'>". (!$_POST['preview'] ? "<input class='button' type='submit' name='preview' value='".ARLAN_28."' />" : "<input class='button' type='submit' name='preview' value='".ARLAN_27."' />")." ". ($sub_action == "edit" || $_POST['editp']? "<input class='button' type='submit' name='update_article' value='".ADLAN_81." ".ARLAN_20."' />\n<input type='hidden' name='content_id' value='$id' />" : ($sub_action == "sa" ? "<input class='button' type='submit' name='sa_article' value='".ARLAN_98."' />" : "<input class='button' type='submit' name='create_article' value='".ADLAN_85." ".ARLAN_20."' />"))."
		</td>
		</tr>
		</table>
		</form>
...
if ($action == "opt") {
	...
	$text = "<div style='text-align:center'>
		...
		<td class='forumheader3' style='width:30%;text-align:center'>". ($pref['article_submit'] ? "<input type='checkbox' name='article_submit' value='1' checked='checked' />" : "<input type='checkbox' name='article_submit' value='1' />")."
		</td>
		</tr>
