...
if ($poll_total = $sql->db_Select("poll")) {
	...
	while ($row = $sql->db_Fetch()) {
		...
		$text .= "<tr>
			...
			<td style='width:20%; text-align:center' class='forumheader3'>". $rs->form_button("button", "main_edit_{$poll_id}", POLLAN_4, "onclick=\"document.location='".e_SELF."?edit.$poll_id'\""). $rs->form_button("submit", "main_delete_{$poll_id}", POLLAN_5, "onclick=\"confirm_($poll_id)\"")."
			</td>
			</tr>";
	}
	$text .= "</table>";
