		...
		*/
		...
		if (!$sql -> db_Query("SHOW COLUMNS FROM ".MPREFIX."links_page")) {
			...
			foreach ($link_cat_export as $link_cat_export_commit) {
				if (!$sql -> db_Insert("links_page_cat", $link_cat_export_commit)) { $links_upd_failed = TRUE; }
			}
			...
			foreach ($link_export as $link_export_commit) {
				if (!$sql -> db_Insert("links_page", $link_export_commit)) { $links_upd_failed = TRUE; }
			}
