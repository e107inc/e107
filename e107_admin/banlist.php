	...
	protected function transferPage()
		{
			...
			if(isset($_POST['ban_import']))  // Got a file to import
			{
				...
				if(!$error) // Got a file of some sort
				{
					...
					{
						$mes->addWarning(BANLAN_IMPORT_NOTHING);
					}

					if($result['duplicates'] > 0)
					{
						$mes->addInfo(str_replace('[x]', $result['duplicates'], BANLAN_IMPORT_DUPLICATES));
					}
					...
					foreach($result['errors'] as $lineNum => $why)
					{
						if(++$shown > 10)
						{
							$mes->addWarning(str_replace('[x]', count($result['errors']) - 10, BANLAN_IMPORT_MORE_SKIPPED));
							break;
						}
						$mes->addWarning(str_replace(array('[x]', '[y]'), array($lineNum, $why), BANLAN_IMPORT_LINE_SKIPPED));
					}

					$summary = e107::getParser()->lanVars(BANLAN_IMPORT_LOG_SUMMARY, array(
						'file'       => $importFile,
