...
$admin_no = $sql -> db_Select("admin", "*", "admin_permissions REGEXP('.A') OR admin_permissions='0' ");
	...
	while(list($admin_id_, $admin_name_, $null, $null, $null, $admin_perms_) = $sql-> db_Fetch()){
		echo $admin_name_." - ".$forum_moderators;
		if(eregi($admin_name_, $forum_moderators)){
			$text .= "<input type=\"checkbox\" name=\"user_reg\" value=\"1\" checked> ".$admin_name_ ."<br />";
		}else{
			$text .= "<input type=\"checkbox\" name=\"user_reg\" value=\"1\"> ".$admin_name_ ."<br />";
		}
	}
