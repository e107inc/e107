...
if($sql -> db_Select("chatbox", "*", "ORDER BY cb_datestamp DESC LIMIT 0, ".$chatbox_posts, $mode="no_where")){
	...
	while(list($cb_id, $cb_nick, $cb_message, $cb_datestamp, $cb_blocked, $cb_ip) = $sql-> db_Fetch()){
		...
		$text .= parsechatbox($CHATBOXSTYLE, $cb_nick, $datestamp, $cb_id, $cb_message, $cb_blocked);
	}
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
function parsechatbox($cbd, $cb_nick, $datestamp, $cb_id, $cb_message, $cb_blocked){
	global $text;
	...
	//	echo htmlentities($tmp[$c])."<br />";
		if(ereg("{|}", $tmp[$c])){
			$var .=  checklayoutcb($tmp[$c], $cb_nick, $datestamp, $cb_id, $cb_message, $cb_blocked);
		}else{
			$var .= $tmp[$c];
		}
	}
...
function checklayoutcb($str, $cb_nick, $datestamp, $cb_id, $cb_message, $cb_blocked){
