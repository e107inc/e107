		...
		if ($cache = $e107cache->retrieve($cachestr)) {
			...
			if ($sql->db_Select("content", "*", "content_id=$sub_action")) {
				...
				if ($totalpages > 1) {
					...
					if ($id != 0) {
						$text .= "<a href='content.php?review.$sub_action.".($id-1)."'>".LAN_25." <<</a> ";
					}
					...
					if (($id+1) != $totalpages) {
						$text .= "<a href='content.php?review.$sub_action.".($id+1)."'>>> ".LAN_26."</a> ";
					}
					if ($epflag) {
						$text .= $textemailprint;
					}
...
function parse_content_article_table($row) {
	...
	if ($totalpages > 1) {
		...
		if ($id != 0) {
			$CONTENT_ARTICLE_PAGES .= "<a href='content.php?article.$sub_action.".($id-1)."'>".LAN_25." &lt;&lt;</a> ";
		}
		for($c = 1; $c <= $totalpages; $c++) {
			$CONTENT_ARTICLE_PAGES .= ($c == ($id+1) ? "<span style='text-decoration: underline;'>$c</span>&nbsp;&nbsp;" : "<a href='content.php?article.$sub_action.".($c-1)."'>$c</a>&nbsp;&nbsp;");
		}
		if (($id+1) != $totalpages) {
			$CONTENT_ARTICLE_PAGES .= "<a href='content.php?article.$sub_action.".($id+1)."'>&gt;&gt; ".LAN_26."</a> ";
		}
		if ($epflag) {
...
function parse_content_article_rating_table($row) {
	...
	if ($comflag) {
		...
		if (!$rater->checkrated("article", $sub_action) && USER) {
			$CONTENT_ARTICLE_RATING .= "<br />\n<div class='smalltext' style='text-align:right'>". $rater->rateselect("&nbsp;&nbsp;&nbsp;&nbsp; ".LAN_40, "article", $sub_action)."</div>";
		}
		else if(USER) {
...
function parse_content_recent_table($row) {
	...
	if (!$rater->checkrated("article", $rev_id) && USER) {
		$CONTENT_RECENT_RATING .= "</span><span class='smalltext' style='width:240px; text-align:right;'>". $rater->rateselect("&nbsp;&nbsp;&nbsp;&nbsp; ".LAN_40, "article", $rev_id)."";
	}
	else if(USER) {
