...
function parse_data($row, $amount, $total, $action_n, $lan) {
	...
	$str .= "<table style='width:95%' class='fborder'>\n<tr>\n<td colspan='4' class='forumheader3'><b>$lan</b> ". ($action == $action_n ? LAN_377 : LAN_378.(($action_n == 6 && ADMIN) || $action_n != 6 ? " ( <a href='".e_SELF."?$action_n'>".LAN_375."</a> )" : ""))."</td>\n</tr>";
	...
	if (!$amount && $amount != 6) {
		while ($data[$c][0]) {
			switch ($action_n) {
				...
				if (eregi("windows", $data[$c][1])) {
					 $image = $imagepath."windows.png";
				}
				else if(eregi("netscape", $data[$c][1])) {
					 $image = $imagepath."netscape.png";
				}
				else if(eregi("konqueror", $data[$c][1])) {
					 $image = $imagepath."konqueror.png";
				}
				else if(eregi("opera", $data[$c][1])) {
					 $image = $imagepath."opera.png";
				}
				else if(eregi("links", $data[$c][1]) || eregi("lynx", $data[$c][1])) {
					 $image = $imagepath."lynx.png";
				}
				else if(eregi("mac", $data[$c][1])) {
					 $image = $imagepath."mac.png";
				}
				else if(eregi("explorer", $data[$c][1])) {
					 $image = $imagepath."explorer.png";
				}
				else if(eregi("firebird", $data[$c][1])) {
					 $image = $imagepath."firebird.png";
				}
				else if(eregi("firefox", $data[$c][1])) {
					 $image = $imagepath."firefox.png";
				}
				else if(file_exists($imagepath.strtolower($data[$c][1])).".png") {
					$image = $imagepath.strtolower($data[$c][1]).".png";
				} else {
					unset($image);
				}
				break;
		...
		for($r = 0; $r <= 9; $r++) {
			if ($data[$c][0]) {
				switch ($action_n) {
					...
					else if((eregi("links", $data[$c][1]) || eregi("lynx", $data[$c][1])) && !eregi("php", $data[$c][1])) {
						 $image = $imagepath."lynx.png";
					}
					else if(eregi("mac", $data[$c][1])) {
						 $image = $imagepath."mac.png";
