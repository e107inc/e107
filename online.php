...
if(MEMBERS_ONLINE){
	...
	while($row = $sql -> db_Fetch()){
		...
		if(!strstr($online_location, "forum_")){ $online_location_page = eregi_replace(".php", "", substr(strrchr($online_location, "/"), 1)); }
		...
		if($online_location_page == "article"){$online_location_page = ARTICLE;}
		...
		if(strstr($online_location, "forum")){
			...
			if(strstr($online_location, "_viewtopic")){
				...
				$sql -> db_Select("forum", "forum_name", "forum_id='$tmp[0]'");
				list($forum['forum_name']) = $sql -> db_Fetch();
		...
		<td class='forumheader3' style='width:50%'><a href='$online_location'>$online_location_page</a></td></tr><br />";
	}
	$text .= "</table></div>";
}
