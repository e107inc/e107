<?php
/*
+ ----------------------------------------------------------------------------+
|     Russian Language Pack for e107 0.7
|     $Revision: 183 $
|     $Date: 2008-08-30 21:41:13 +0600 (Сб, 30 авг 2008) $
|     $Author: yarodin $
+----------------------------------------------------------------------------+
*/
define("LANHELP_1", "Чёрный");
define("LANHELP_2", "Голубой");
define("LANHELP_3", "Коричневый");
define("LANHELP_4", "Циан");
define("LANHELP_5", "Синий");
define("LANHELP_6", "Бордовый");
define("LANHELP_7", "Зелёный");
define("LANHELP_8", "Индиго");
define("LANHELP_9", "Оливковый");
define("LANHELP_10", "Оранжевый");
define("LANHELP_11", "Красный");
define("LANHELP_12", "Фиолетовый");
define("LANHELP_13", "Белый");
define("LANHELP_14", "Жёлтый");
define("LANHELP_15", "Мелкий");
define("LANHELP_16", "Маленький");
define("LANHELP_17", "Нормальный");
define("LANHELP_18", "Большой");
define("LANHELP_19", "Крупный");
define("LANHELP_20", "Очень большой");
define("LANHELP_21", "Нажмите, чтобы открыть диалог выбора цвета ...");
define("LANHELP_22", "Нажмите, чтобы открыть диалог выбора размера ...");
define("LANHELP_23", "Вставить ссылку: [link]http://example.com[/link] или  [link=http://example.com]Перейти к сайту[/link]");
define("LANHELP_24", "Жирный текст: [b]Этот текст будет жирным[/b]", "font-weight:bold; width: 20px");
define("LANHELP_25", "Наклонный текст: [i]Этот текст будет наклонным[/i]", "font-style:italic; width: 20px");
define("LANHELP_26", "Подчёркнутый текст: [u]Это текст будет подчёркнутым[/u]", "text-decoration: underline; width: 20px");
define("LANHELP_27", "Вставить изображение: [img]image.jpg[/img]");
define("LANHELP_28", "Разместить по центру: [center]Этот текст будет размещён по центру[/center]");
define("LANHELP_29", "Разместить слева: [left]Этот текст будет размещён слева[/left]");
define("LANHELP_30", "Разместить справа: [right]Этот текст будет размещён справа[/right]");
define("LANHELP_31", "Цитировать текст: [blockquote]Этот текст будет процитирован (indented)[/blockquote]");
define("LANHELP_32", "Код - предформированный код: [code]\$foo = bah;[/code]");
define("LANHELP_33", "HTML - удаляет пробелы между строчками: [html]<table><tr><td> etc[/html]");
define("LANHELP_34", "Вставить разделитель страницы");
define("LANHELP_35", "адрес гиперссылки");
define("LANHELP_36", "Несортированный список: [list]line1*line2*line3[/list] Сортированный список: [list=type]line1*line2*line3[/list]");
define("LANHELP_37", "Вставить изображение из каталога e107_images/newspost_images/");
define("LANHELP_38", "ссылка на полное изображение будет сгенерирована");
define("LANHELP_39", "Вставить ссылку загрузки из существующих загрузок");
define("LANHELP_40", "В настоящее время нет существующих загрузок");
define("LANHELP_41", "Размер шрифта...");
define("LANHELP_42", "Выберите изображение...");
define("LANHELP_43", "Выберите файл...");
define("LANHELP_44", "Щелкните, чтобы открыть/закрыть диалог смайликов ...");
define("LANHELP_45", "Вставить изображении из каталога: ");
define("LANHELP_46", "* Не найдено файлов в: ");
define("LANHELP_47", "Вставить flash: [flash=width,height]http://www.example.com/file.swf[/flash]");
define("LANHELP_48", "Вставить YouTube video: [youtube=tiny|small|medium|big|huge|width,height]6kYjxJmk0wc[/youtube]");
define("LANHELP_49", "Параграф: [p=class name]Добавьте Ваш текст[/p]");
define("LANHELP_50", "Заголовок: for H2 [h]Текст Заголовка[/h] или [h=2]Текст Заголовка[/h]");
define("LANHELP_51", "Отключите HTML строки  для закрытого текста: [nobr] текст [/nobr]");
define("LANHELP_52", "Новая строка (HTML): [br]");
define("LANHELP_53", "Выровнять по ширине:\n[justify]Этот текст будет выровнен по всей ширине[/justify]");
define("LANHELP_54", "HTML блок (div tag): [block]Ваш контент[/block]");


?>