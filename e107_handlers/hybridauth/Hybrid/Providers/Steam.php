...
class Hybrid_Providers_Steam extends Hybrid_Provider_Model_OpenID
{
    ...
    function getUserProfileLegacyAPI()
    {
		...
		$this->user->profile->displayName		=	(array_key_exists('steamID',$data))?$data['steamID']:"";//property_exists($data, 'steamID') ? (string)$data->steamID : '';
		$this->user->profile->description		=	"";//property_exists($data, 'summary') ? (string)$data->summary : '';
		$this->user->profile->firstName			=	(array_key_exists('realname',$data))?$data['realname']:"";//property_exists($data, 'realname') ? (string)$data->realname : '';
