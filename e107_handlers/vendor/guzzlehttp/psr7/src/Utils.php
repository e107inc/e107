...
final class Utils
{
     ...
     * @return string
     */
    public static function asciiToLower($string)
    {
     ...
     * @return string
     */
    public static function asciiToUpper($string)
    {
        return strtr($string, 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ');
    }
     ...
     * @return string
     */
    public static function asciiUcFirst($string)
    {
        if ($string === '') {
            return '';
        }
     ...
     * @return bool
     */
    public static function caselessContains($haystack, $needle)
    {
     ...
     * @return bool
     */
    public static function caselessEquals($left, $right)
    {
     ...
     * @return bool
     */
    private static function writeAll(StreamInterface $dest, $buf)
    {
        $written = 0;
        $len = strlen($buf);

        while ($written < $len) {
            $result = $dest->write((string) substr($buf, $written));
            if ($result <= 0) {
                return false;
            }
     ...
     * @return void
     */
    private static function warnOnInvalidModifyRequestChanges(array $changes)
    {
     ...
     */
    private static function warnOnInvalidModifyRequestChange($key, $expected, $value)
    {
        \trigger_deprecation(
            'guzzlehttp/psr7',
            '2.11',
