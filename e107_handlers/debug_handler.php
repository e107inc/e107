...
class e107_debug {
	...
	function set_error_reporting() {
		if ($this->debug_level & 32768) {
			// Debug constants are defined in class2 for now
...
class e107_db_debug {
	...
	var $sDBdbg;
	// DB debug string (deprecated)
	var $aSQLdetails;
	// DB query analysis (in pieces for further analysis)
	var $aDBbyTable;
	var $aOBMarks;
	// Track output buffer level at each time mark
	var $aMarkNotes; // Other notes can be added and output...
	var $aTimeMarks; // Overall time markers
	var $curTimeMark;
	var $nTimeMarks;
	// Provide an array index for time marks. Stablizes 'current' function
	var $aTrafficCounts;
	...
	function Mark_Query($query, $rli, $aTrace) {
		...
		$sQryRes = is_null($rli) ? mysql_query("EXPLAIN $query") :
		 mysql_query("EXPLAIN $query", $rli);
		$nFields = "";
		if ($sQryRes) {
		...
		$this->aSQLdetails[$iQuery]['ok'] = $sQryRes ? TRUE :
		 FALSE;
		$this->aSQLdetails[$iQuery]['error'] = $sQryRes ? '' :
		 mysql_error();
