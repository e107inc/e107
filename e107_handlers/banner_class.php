...
class banner{
		function getBanner($bannerquery) {
				...
				while($row = $sql -> db_Fetch()){
					...
					if(!$menu_pref['banner_visibilitytype'] || $menu_pref['banner_visibilitytype'] == "1"){		//use individual banner visibility
					...
					}elseif($menu_pref['banner_visibilitytype'] == "2"){		//use campaign visibility
						if(preg_match("#\^#", $banner_campaign)){
							$campaignsplit = explode("^", $banner_campaign);
							$banner_campaign = $campaignsplit[0];
						}
					...
					if(check_class($menu_pref[$classcheck])){					//class check for campaign
						if(check_class($banner_active)){						//class check for banner
