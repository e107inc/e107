...
class e_form
{
	...
	function avatarpicker($name,$curVal='',$options=array())
	{
		...
		foreach($avFiles as $fi)
		{
			...
			$text .= "\n<a class='e-expandit' title='".IMALAN_170."' href='#{$optioni}'><img src='".$img_path."' alt=''  onclick=\"insertext('".$fi['fname']."', '".$idinput."');document.getElementById('".$previnput."').src = this.src;return false\" /></a> ";
			$count++;
		...
		{
			$text .= "<div class='row'>";
			$text .= "<div class='alert alert-info'>".IMALAN_169."</div>"; //TODO LAN

			if(ADMIN)
			{
				$text .= "<div class='alert alert-danger'>".IMALAN_171." <b>".e_AVATAR_DEFAULT."</b> ".IMALAN_172."</div>"; //TODO LAN
			}
