 ...
 * or when a caller asks {@see e_random} for something it cannot produce.
 */
class e_random_exception extends Exception
{
}
 ...
 * CSPRNG at all, every method throws {@see e_random_exception} and the request
 * fails, because a guessable secret is worse than an unavailable site.
 *
 * {@see e_random::isAvailable()} exists so the installer and the upgrade
 ...
 *
 * The public surface is kept identical to the e107\Security\Random class on
 * master (aliased there to `e_random`), so a fix can be moved between the
 * branches unchanged.
 */
class e_random
{
	...
	public static function bytes($length)
	{
		...
		if($length < 1)
		{
			throw new e_random_exception('e_random::bytes() requires a length of at least 1 byte.');
		}
		...
		{
			throw new e_random_exception(self::UNAVAILABLE, 0, $e);
		}
	}
	...
	public static function hex($length)
	{
		...
		if($length < 1)
		{
			throw new e_random_exception('e_random::hex() requires a length of at least 1 character.');
		}
	...
	public static function int($min, $max)
	{
		...
		if($min > $max)
		{
			throw new e_random_exception('e_random::int() requires $min to be no greater than $max.');
		}
	 ...
	 * @return mixed
	 * @throws e_random_exception when no CSPRNG is available, or $values is empty
	 */
	public static function pick(array $values)
	{
		if(empty($values))
		{
			throw new e_random_exception('e_random::pick() requires a non-empty array.');
		}
	...
	private static function assertAvailable()
	{
		...
		throw new e_random_exception(self::UNAVAILABLE);
	}
}
