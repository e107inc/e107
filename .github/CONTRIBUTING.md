...
- **Raise a bundled plugin's version when you change it.**  For a plugin shipped with core, the `version` attribute in its `plugin.xml` is very nearly the only signal a site has that the plugin changed at all, so a pull request touching `e107_plugins/<name>/` raises that plugin's version above the one at the last tagged release, and moves its `date` up to the day of your change.  Once per release cycle is enough: if the version has already moved since that tag, leave it where it is.  CI checks this, and so can you: `e107_tests/_tools/plugin-metadata/bin/plugin-metadata-check --base=origin/master` reaches the same verdict locally, with `--base=origin/release/v2.3.x` for a branch destined there.
