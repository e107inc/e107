...
if (defined("CORE_PATH") && ($page == "index.php" || !$page)) {
	$page = "news.php";
}
...
function e107_parse($text, $referrer) {
	...
	for ($i = 0; $i < count($matches); $i++) {
		...
		if (!function_exists($parse_func)) {
			$parse_file = ('CORE' == $p[0]) ? e_HANDLER."parse/" :
			e_PLUGIN.$p[0]."/parse/";
...
function online() {
	...
	if (USER) {
		...
		if ($sql->db_Select("online", "*", "(online_ip='{$ip}' AND online_user_id = '0') OR online_user_id = '{$udata}'")) {
			$row = $sql->db_Fetch();
			extract($row);
			if ($online_user_id == $udata) {
				//Matching user record
				if ($online_timestamp < (time() - $online_timeout)) {
...
function html($string) {
	...
	for ($a = 0; $a < $match_count; $a++) {
		...
		$after_replace = str_replace("[e_NL]", "", $after_replace);
...
//	$string = str_replace($search, $replace, $string);
	...
	for ($a = 0; $a < $match_count; $a++) {
		...
		$after_replace = str_replace("[e_NL]", "\n", $after_replace);
		...
		if (strpos($result[2][$a], 'highlight')) {
			...
			$class = (strlen(trim($result[2][$a]))) ? $result[2][$a] :
			'indent';
		}
