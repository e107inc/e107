...
global $LINK_MAIN_HEADING, $total_links_cat, $row;
return (!$total_links_cat ? $row['link_category_name'] : "<a href='links.php?cat.".$row['link_category_id']."'>".$row['link_category_name']."</a>");
SC_END

SC_BEGIN LINK_MAIN_DESC
global $LINK_MAIN_DESC, $row, $linkspage_pref;
return ($linkspage_pref['link_cat_desc'] ? $row['link_category_description'] : "");
SC_END

SC_BEGIN LINK_MAIN_NUMBER
global $LINK_MAIN_NUMBER, $total_links_cat, $linkspage_pref;
if($linkspage_pref['link_cat_amount']){
...
$LINK_MAIN_ICON = "";
if($linkspage_pref['link_cat_icon']){
	if ($row['link_category_icon']) {
		...
		if($linkspage_pref['link_cat_icon_empty']){
...
if($linkspage_pref['link_cat_total']){
...
return ($linkspage_pref['link_cat_total'] ? "<a href='".e_PLUGIN."links_page/links.php?cat.all'>".LAN_67."</a>" : "");
SC_END

SC_BEGIN LINK_MAIN_TOPREFER
global $LINK_MAIN_TOPREFER, $linkspage_pref;
return ($linkspage_pref['link_cat_toprefer'] ? "<a href='".e_PLUGIN."links_page/links.php?top'>".LAN_LINKS_12."</a>" : "");
SC_END

SC_BEGIN LINK_MAIN_TOPRATED
global $LINK_MAIN_TOPRATED, $linkspage_pref;
return ($linkspage_pref['link_cat_toprated'] ? "<a href='".e_PLUGIN."links_page/links.php?rated'>".LAN_LINKS_13."</a>" : "");
SC_END
...
if($linkspage_pref['link_icon']){
	if ($row['link_button']) {
		...
		if($linkspage_pref['link_icon_empty']){
...
return ($linkspage_pref['link_url'] ? $row['link_url'] : "");
SC_END

SC_BEGIN LINK_CAT_REFER
global $LINK_CAT_REFER, $linkspage_pref, $row;
return ($linkspage_pref['link_referal'] ? LAN_88." ".$row['link_refer'] : "");
SC_END
...
if ($qs[0] != "top" && $linkspage_pref['link_submit'] && check_class($linkspage_pref['link_submit_class'])) {
