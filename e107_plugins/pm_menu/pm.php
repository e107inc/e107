...
if ($pref['pm_userclass'] && !check_class($pref['pm_userclass'])) {
	$notallowed = 1;
}
if (ADMINPERMS == 0) {
	$notallowed = 0;
...
if (eregi(PMLAN_9, $_POST['postpm'])) {
	...
	if ($_POST['sendtype'] == "userclass") {
		...
		if ($notfound) {
			...
			header("location:pm.php?send,mb.".$_POST['to']);
			 exit;
		} else {
			header("location:pm.php?read,ms.".$_POST['to']);
			 exit;
		}
	}
}
...
if (preg_match("/view\./", PM_QUERY)) {
	...
	if ($msgtext) {
		switch($msgtext) {
			case("MARK"):
			header("location:pm.php?".e_QUERY);
			 exit;
			break;
			case("FAIL"):
			require_once(HEADERF);
...
if (preg_match("/^block\./", PM_QUERY)) {
	...
	if ($pm->block_user($tmp[1], USERNAME)) {
		...
		header("location:pm.php?read,bp.".$tmp[1]);
		 exit;
	}
}
...
if (preg_match("/^unblock\./", PM_QUERY)) {
	...
	if ($pm->is_blocked($tmp[1], USERNAME)) {
		if ($pm->unblock_user($tmp[1], USERNAME)) {
			header("location:pm.php?vb,ub.".$tmp[1]);
			 exit;
		} else {
			header("location:pm.php?vb".$tmp[1]);
			 exit;
		}
	} else {
		header("location:pm.php?vb,ubnf.".$tmp[1]);
		 exit;
	}
}
