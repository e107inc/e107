...
if (strstr(($_SERVER['PHP_SELF'] ? $_SERVER['PHP_SELF'] : $_SERVER['SCRIPT_FILENAME']), "headlines_menu.php")) {
	...
	if ($feeds = $sql->db_Select("headlines", "*", "headline_active='1' ")) {
		...
		$gen = new convert;
		 $datestamp = $gen->convert_date(($headline_timestamp ? $headline_timestamp : time()), "long");
		$text .= "</td></tr>
...
if ($sql->db_Select("headlines", "*", "headline_active='1' ")) {
	...
	$gen = new convert;
	 $datestamp = $gen->convert_date(($headline_timestamp ? $headline_timestamp : time()), "short");
