...
class forum_event
{
	...
	function forum_eventlogin($data) // Clear user_plugin_forum_viewed on user LOGIN
	{
		...
		echo('hola');
		print_a($data);*/
		e107::getDb()->update('user_extended', "user_plugin_forum_viewed = NULL WHERE user_extended_id = ".(int) $data['user_id']);
	...
	function forum_eventnewpost($data) // Remove thread id from user_plugin_forum_viewed when a new reply is posted
	{
		...
		e107::getDb()->update('user_extended', "user_plugin_forum_viewed = TRIM(BOTH ',' FROM REPLACE(CONCAT(',', user_plugin_forum_viewed, ','), ',".(int) $data['post_thread'].",', ',')) WHERE FIND_IN_SET('".(int) $data['post_thread']."', user_plugin_forum_viewed) AND user_extended_id != ".(int) $data['post_user']);
