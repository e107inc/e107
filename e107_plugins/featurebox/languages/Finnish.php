<?php
/*
+ ----------------------------------------------------------------------------+
|     e107 website system
|
|     쯘teve Dunstan 2001-2002
|     http://e107.org
|     jalist@e107.org
|
|     Released under the terms ja conditions of the
|     GNU General Public License (http://gnu.org).
|
|     $Source: /cvs_backup/e107_langpacks/e107_plugins/featurebox/languages/Finnish.php,v $
|     $Revision: 1.2 $
|     $Date: 2006-01-14 22:26:04 $
|     $Kirjoittaja: stevedunstan $
+----------------------------------------------------------------------------+
*/

define("FBLAN_01", "Feature Box");
define("FBLAN_02", "This plugin allows you to display a box above your news items with features / whatever you like in it. The messages can either be revolved randomly or dynamically faded.");
define("FBLAN_03", "Configure feature box");
define("FBLAN_04", "Feature Box plugin has been successfully installed. To add messages ja configure, return to the main admin page ja click on the feature box icon in the plugin section.");
define("FBLAN_05", "No feature box messages defined yet");
define("FBLAN_06", "Existing Feature Box Messages");
define("FBLAN_07", "Otsikko / Otsikko");
define("FBLAN_08", "Message text");
define("FBLAN_09", "N채kyvyys of message");
define("FBLAN_10", "Create Feature Box Message");
define("FBLAN_11", "P채ivit채 Feature Box Message");
define("FBLAN_12", "Mode");
define("FBLAN_13", "Randomly revolve messages");
define("FBLAN_14", "N채yt채 this message only");
define("FBLAN_15", "Message added to tietokanta.");
define("FBLAN_16", "Message updated in tietokanta.");
define("FBLAN_17", "Field(s) left blank");
define("FBLAN_18", "Feature Box Message poistettu");
define("FBLAN_19", "Options");
define("FBLAN_20", "Muokkaa");
define("FBLAN_21", "Poista");
define("FBLAN_22", "Render type");
define("FBLAN_23", "In theme box");
define("FBLAN_24", "Plain");
define("FBLAN_25", "Template");
define("FBLAN_26", "you can use a different template for each message, add templates to e107_plugins/featurebox/templates/ folder");

?>
