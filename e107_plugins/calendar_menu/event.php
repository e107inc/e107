...
if ($action == "ne" || $action == "ed")
{
	if (check_class($pref['eventpost_admin']) || getperms('0'))
	{
		...
		$text = "<form method='post' action='".e_SELF."' name='linkform'>
			<table style='width:100%' class='fborder' align='center'>";
		...
		if (ADMIN == TRUE && $action != "ed") {
			...
			$text .= "<tr>
				...
				<td class='forumheader3' style='width:80%'>".EC_LAN_54." <input class='tbox' type='text' name='ne_new_category' size='30' value='$ne_new_category' maxlength='100' style='width:95%' /> ";
