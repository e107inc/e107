...
| 01.02.06 - Mod corrected
| 09.06.06 - Query simplified to just pick out event start dates.
+----------------------------------------------------------------------------+
*/
...
if (check_class($pref['eventpost_super']))
{
    $cal_qry = "SELECT e.event_rec_m, e.event_rec_y, e.event_start, e.event_end, ec.*
	FROM #event as e LEFT JOIN #event_cat as ec ON e.event_category = ec.event_cat_id
	WHERE (e.event_start >= {$cal_monthstart} AND e.event_start <= {$cal_monthend})  OR (e.event_rec_y = {$cal_current_month}) order by e.event_start";
}
else
{
    $cal_qry = "SELECT e.event_rec_m, e.event_rec_y, e.event_start, e.event_end, ec.*
