...
//  ----------------------------------------------------------------------------
//
// Some small changes, tagged --**e107**--, to simplify use with the e107 content management system
//
...
//require_once(dirname(__FILE__).'/config/tcpdf_config.php');		--**e107**--
	...
	*/
	class TCPDF {
		...
		*/
		 ...
		 */
		protected $cell_height_ratio = 1.25;			// --**e107**-- K_CELL_HEIGHT_RATIO;
			...
			while ($elkey < $maxel) {
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{0} == '/') {
						...
						// get attributes
						// Need to pick out attributes in single quotes as well  								--**e107**--
		...
		protected function openHTMLTagHandler(&$dom, $key, $cell=false) {
			...
			switch($tag['value']) {
				...
				case 'img': {
					...
					{
						if (method_exists($this, 'filePathModify'))		// --**e107**--
						{
							$tag['attribute']['src'] = $this->filePathModify($tag['attribute']['src'],'img');
						}
						else
						{
