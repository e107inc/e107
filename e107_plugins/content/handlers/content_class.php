...
*/
		...
		*/		
		...
		function drawBreadcrumbFromUrl($page="", $query="", $base="", $nolink=""){
				...
				//if($base == "base"){
					if($nolink){
						$basecrumb = CONTENT_LAN_58." ".$sep." ".CONTENT_LAN_59." ".$sep;
					}else{
						$basecrumb = "<a href='".e_BASE."'>".CONTENT_LAN_58."</a> ".$sep." <a href='".e_PLUGIN."content/content.php'>".CONTENT_LAN_59."</a> ".$sep;
					}
				//}
				if($page == "content_submit.php" || $page == "content_manager.php"){
					$crumb = $basecrumb;
				}else{
					return FALSE;
				}
				...
				if($page == "content.php"){
					$crumb = $basecrumb;
					$crumb .= " ".$sep." ".($nolink ? CONTENT_LAN_60 : "<a href='".e_SELF."?type.".$type_id."'>".CONTENT_LAN_60."</a>");
				}
				*/

				$crumb = substr($crumb,0,-strlen($sep)-1);
				return $crumb;
		}
		...
		function getOrder(){
				...
				if(substr($action,0,5) == "order"){
					...
					$thisid = (isset($type_id) ? $type_id : "0");
					$orderstring = ($content_pref["content_defaultorder_{$thisid}"] ? $content_pref["content_defaultorder_{$thisid}"] : "orderddate" );
				}
