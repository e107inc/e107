<?php
/*
 * Dutch Language File for the
 *   e107 website system (http://e107.org).
 * Released under the terms and conditions of the
 *   GNU General Public License v3 (http://gnu.org).
 * $HeadURL$
 * $Revision$
 * $Date$
 * $Author$
 */

define('ADSTAT_ON',  'Aan');
define('ADSTAT_OFF', 'Uit');

define('ADSTAT_L1',  'Deze plugin logt alle bezoeken aan je site en creëert statistiekoverzichten gebaseerd op de verzamelde gegevens.');
define('ADSTAT_L2',  'De statistiek logger is succesvol geïnstalleerd. Ga naar het configuratiescherm om te activeren en klik daar op Activeren.<br /><b>De permissies van de e107_plugins/log/logs directory moeten zijn ingesteld op 777 (chmod 777)</b>'); //@FIXME: chmod
define('ADSTAT_L3',  'Statistiek logging');
define('ADSTAT_L4',  'Activeren statistiek logging');
define('ADSTAT_L5',  'Statistiek soorten');
define('ADSTAT_L6',  'Browsers');
define('ADSTAT_L7',  'Besturings Systems');
define('ADSTAT_L8',  'Scherm resolutie / kleurdiepte');
define('ADSTAT_L9',  'Landen/domeinen van bezoekers');
define('ADSTAT_L10', 'Verwijzers');
define('ADSTAT_L11', 'Zoekopdrachten');
define('ADSTAT_L12', 'Terugzetten tellers');
define('ADSTAT_L13', 'Hiermee zet je tellers terug op 0 - pas dus op!');
define('ADSTAT_L14', 'Pagina telling');
define('ADSTAT_L15', 'Bijwerken statistiek instellingen');
define('ADSTAT_L16', 'Site statistieken instellingen');
define('ADSTAT_L17', 'Statistiek instellingen bijgewerkt');
define('ADSTAT_L18', 'Sta bezoek van de hoofd statistiekpagina toe voor ...');
define('ADSTAT_L19', 'Recente bezoekers');
define('ADSTAT_L20', 'Tel beheerbezoeken');
define('ADSTAT_L21', 'Max aantal te tonen records op statistiekpagina');
define('ADSTAT_L22', 'Start bijwerkfunctie');
define('ADSTAT_L23', 'logs van een voorgaande versie van e107 zijn nog aanwezig, werk ze hier bij');
define('ADSTAT_L24', 'Ga naar het bijwerkscript');
define('ADSTAT_L25', 'Geselecteerde statistieken op nul gesteld');
define('ADSTAT_L26', 'Verwijder pagina regels');
define('ADSTAT_L27', 'als je stats onjuiste pagina’s bevat, kun je ze hier verwijderen');
define('ADSTAT_L28', 'Open pagina');
define('ADSTAT_L29', 'Paginanaam');
define('ADSTAT_L30', 'Aankruisen om te verwijderen');
define('ADSTAT_L31', 'Verwijder geselecteerde pagina’s');
define('ADSTAT_L32', 'Pagina schonen');
define('ADSTAT_L33', 'Configureren statistiek logging');
define('ADSTAT_L34', 'Site Stats');
define('ADSTAT_L38', 'De e107_plugins/log/logs/ map moet beschrijfbaar zijn, chmod deze naar 777'); //@FIXME: chmod

?>