<?php
/*
+---------------------------------------------------------------+
|	e107 website system
|
|	©Steve Dunstan 2001-2002
|	http://e107.org
|	jalist@e107.org
|
|	Released under the terms and conditions of the
|	GNU General Public License (http://gnu.org).
+---------------------------------------------------------------+
*/

define("PAGE_NAME", "Chatbox");
define("CHATBOX_L1", "Kan dit bericht niet plaatsen omdat de gebruikersnaam is gereserveerd voor een lid - als dit uw gebruikersnaam is, logt u dan eerst in.");
define("CHATBOX_L2", "Chatbox");
define("CHATBOX_L3", "U moet zijn ingelogd om commentaar te kunnen plaatsen - logt u in of, als u nog geen lid bent, meld u dan <a href="signup.php">hier</a> aan");
define("CHATBOX_L4", "Plaats");
define("CHATBOX_L5", "Herstel");
define("CHATBOX_L6", "[geblokkeerd door beheerder]");
define("CHATBOX_L7", "Deblokkeer");
define("CHATBOX_L8", "Info");
define("CHATBOX_L9", "Blokkeer");
define("CHATBOX_L10", "Verwijder");
define("CHATBOX_L11", "Nog geen berichten.");
define("CHATBOX_L12", "Bekijk alle berichten");
define("CHATBOX_L13", "modereer chatbox");
define("CHATBOX_L14", "Emoticons");
define("CHATBOX_L15", "Bericht te lang, of er is een leeg bericht geplaatst");
define("CHATBOX_L16", "Anoniem");
define("CHATBOX_L17", "Dubbelpost");
define("CHATBOX_L18", "Chatbox berichten gemodereerd");
define("CHATBOX_L19", "U kunt slechts één keer in de ".FLOODTIMEOUT." seconden een bericht plaatsen");

define("CHATBOX_L20", "Chatbox (alle berichten)");
define("CHATBOX_L21", "Chat berichten");
define("CHATBOX_L22", "op");
define("CHATBOX_L23", "Fout!");
define("CHATBOX_L24", "U bezit niet de juiste rechten om deze pagina te bekijken.");
define("CHATBOX_L25", "[ dit bericht is door een beheerder geblokkeerd ]");

define("NT_LAN_CB_1", "Chatbox gebeurtenissen");
define("NT_LAN_CB_2", "Bericht geplaatst");
define("NT_LAN_CB_3", "Geplaatst door");
define("NT_LAN_CB_4", "IP adres");
define("NT_LAN_CB_5", "Bericht");
define("NT_LAN_CB_6", "Chatbox bericht geplaatst");

?>