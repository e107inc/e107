...
class login_menu_class
{
    ...
    function get_forum_stats($get_stats=true) 
	{
        ...
        if($get_stats && is_readable(e_PLUGIN.'forum/forum_class.php')) {
            ...
            $lbox_stats[0]['stat_new'] = $sql->count('forum_thread', '(*)',
                "WHERE thread_datestamp > " . (int) defset('USERLV', 0)
    ...
    function get_chatbox_menu_stats($get_stats=true) 
	{
        ...
        if(!empty($get_stats))
        {
            $lbox_stats[0]['stat_new']  = $sql->count('chatbox', '(*)', 'WHERE `cb_datestamp` > '.(int) defset('USERLV', 0));
        }
