...
class download_request
{
	...
	static function findByName($request)
	{
		...
		{
			$candidates[$k] = "'" . $sql->escape($candidate, false) . "'";
		}

		if($sql->select('download', 'download_id', 'download_url IN (' . implode(',', $candidates) . ') LIMIT 1'))
		{
			return $sql->fetch();
		}
		...
		$like = $sql->escape('%/' . addcslashes($request, '%_\\'), false);

		if($sql->select('download', 'download_id, download_url', "download_url LIKE '" . $like . "' LIMIT 25"))
		{
			while($candidate = $sql->fetch())
			{
	...
	//	}
		...
		if(!empty($table) && in_array($table, array('download', 'upload'), true)) // validate dynamic table name fail-closed
		{
			if($table === 'download')
			{
				$qry = "SELECT t.*, dc.download_category_class FROM `#download` AS t
					LEFT JOIN `#download_category` AS dc ON dc.download_category_id = t.download_category
					WHERE t.download_id = ".intval($id);
			}
			else
			{
				$qry = "SELECT t.* FROM `#upload` AS t WHERE t.upload_id = ".intval($id);
			}

			$row = $sql->gen($qry) ? $sql->fetch() : false;
