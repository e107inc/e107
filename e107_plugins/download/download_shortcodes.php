...
if($parm == "request"){
	...
	if($row['download_mirror_type']){
		$text = ($pref['agree_flag'] ? "<a href='".e_PLUGIN."download/download.php?mirror.".$row['download_id']."' onclick= \"return confirm('{$agreetext}');\">" : "<a href='".e_PLUGIN."download/download.php?mirror.".$row['download_id']."' title='".LAN_dl_32."'>");
	}else{
...
return  "<a href='".e_PLUGIN."download/download.php?view.".$row['download_id']."'>".$tp->toHTML($row['download_name'],TRUE,'LINKTEXT')."</a>";
SC_END
...
if($parm == "link"){
	return "<a href='".e_PLUGIN."download/download.php?view.".$row['download_id']."' >".$img."</a>";
}else{
	return $img;
}
return;
...
$agreetext = $tp->toJS($tp->toHTML($pref['agree_text'],FALSE,'DESCRIPTION'));
	if($row['download_mirror_type']){
		return ($pref['agree_flag'] ? "<a href='".e_PLUGIN."download/download.php?mirror.".$row['download_id']."' onclick= \"return confirm('{$agreetext}');\">" : "<a href='".e_PLUGIN."download/download.php?mirror.".$row['download_id']."' >");
	}else{
...
global $dl;
$link['view'] = "<a href='".e_PLUGIN."download/download.php?view.".$dl['download_id']."'>".$dl['download_name']."</a>";
...
if($dl['download_mirror'])
{
	...
	{
		return "<a href='".e_PLUGIN."download/download.php?mirror.".$dl['download_id']."'>".LAN_dl_66."</a>";
	}
	else
	{
...
global $dl,$pref;
return (check_class($pref['download_reportbroken'])) ? "<a href='".e_PLUGIN."download/download.php?report.".$dl['download_id']."'>".LAN_dl_45."</a>" : "";
SC_END
...
global $dl,$sql;
	...
	if ($sql->db_Select("download", "*", "download_category='".intval($dl['download_category_id'])."' AND download_id < {$dl_id} AND download_active > 0 && download_visible IN (".USERCLASS_LIST.") ORDER BY download_datestamp DESC LIMIT 1")) {
		$row = $sql->db_Fetch();
		return "<a href='".e_PLUGIN."download/download.php?view.".$row['download_id']."'>&lt;&lt; ".LAN_dl_33." [".$row['download_name']."]</a>\n";
	} else {
		return "&nbsp;";
	}
SC_END
...
$dl_id = intval($dl['download_id']);
	if ($sql->db_Select("download", "*", "download_category='".intval($dl['download_category_id'])."' AND download_id > {$dl_id} AND download_active > 0 && download_visible IN (".USERCLASS_LIST.") ORDER BY download_datestamp ASC LIMIT 1")) {
		$row = $sql->db_Fetch();
		 extract($row);
		return "<a href='".e_PLUGIN."download/download.php?view.".$row['download_id']."'>[".$row['download_name']."] ".LAN_dl_34." &gt;&gt;</a>\n";
	} else {
		return "&nbsp;";
	}
SC_END
...
global $dl;
return "<a href='".e_PLUGIN."download/download.php?list.".$dl['download_category']."'>".LAN_dl_35."</a>";
SC_END
