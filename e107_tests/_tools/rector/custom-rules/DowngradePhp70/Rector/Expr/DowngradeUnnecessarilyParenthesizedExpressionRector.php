...
namespace E107\Rector\DowngradePhp70\Rector\Expr;
...
final class DowngradeUnnecessarilyParenthesizedExpressionRector extends AbstractRector
{
     ...
     *
     * FuncCall is deliberately excluded: stripping the parentheses from an
     * invoked callee such as `($callable)(...)` parses on PHP 5 but is still a
     * runtime error when $callable holds an array callable (`[$obj, $method]`),
     * which only PHP 7+ can invoke directly. That whole case is owned by
     * {@see \E107\Rector\DowngradePhp70\Rector\FuncCall\DowngradeUncallableValueCallToCallUserFuncRector},
     * which rewrites it to a runtime-safe call_user_func(). The accesses below
     * (property, array, static) are not invocations, so de-parenthesizing them
     * is always safe on PHP 5.
     */
...
CODE_SAMPLE
    ...
    /**
     * @param ArrayDimFetch|PropertyFetch|MethodCall|StaticPropertyFetch|StaticCall $node
     */
