...
class ContactFormCest
{
	 ...
	 * the fixture mints a pair through the handler and the tester's cookie jar
	 * carries the session on to the submission. Without this there is no way to
	 * send a request that is legitimate in every respect except the one under
	 * test, and no way to tell a fix from a form that no longer works.
	 *
	 * @param AcceptanceTester $I
	 ...
	 *
	 * The public code is in the markup and its answer is in the session the
	 * same visitor holds, so a test can answer the CAPTCHA it was given rather
	 * than minting a fresh one out of the application. That is the only way to
	 * measure the fields the template produced.
	 *
	 * @param AcceptanceTester $I
	...
	private function captchaFromSource(AcceptanceTester $I, $source)
	{
		...
		$I->assertNotEmpty($code,
			'the session must hold the answer to the CAPTCHA the form was rendered with');
	...
	/**
	 * The address the visitor typed reaches the recipient inside the message, not only as a Reply-To header.
	 */
...
switch(\$act)
{
	...
	case 'captcha':
		// The answer is kept server side against the public code, so a valid
		// pair is minted here and the caller's session carries the answer.
	...
	case 'captchasecret':
		// The answer to a code the application already rendered, read out of
		// the session it was stored in.
		\$store = e107::getSession('secureImage')->get('secret');
		\$rand = isset(\$_GET['rand']) ? \$_GET['rand'] : '';
		echo "PROBE_OK\n";
		echo "CODE=".(isset(\$store[\$rand]) ? \$store[\$rand] : '')."\n";
		break;
