...
switch(\$act)
{
	...
	case 'poll':
		...
		\$id = \$sql->createQueryBuilder()->insert('polls')->insertGetId(array(
			'poll_datestamp'       => time(),
	...
	case 'votes':
		\$row = \$sql->createQueryBuilder()->select('poll_votes')->from('polls')
			->where('poll_id', (int) \$_GET['id'])->fetchRow();
