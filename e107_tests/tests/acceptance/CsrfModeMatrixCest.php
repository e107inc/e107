...
class CsrfModeMatrixCest
{
	...
	public function _before(AcceptanceTester $I)
	{
		...
		// that could carry Fetch Metadata at all. A browser appends Sec-Fetch-Site
		// only to a potentially trustworthy origin, so on a plain-HTTP site it
		// never arrives and e_session::tokenCheckMode() softens the browser-only
		// modes to the hybrid. Without this the strict expectations below would be
		// asserting against mode 3 wearing mode 4's name. The header is not a
		// fiction: it is exactly what a proxy adds, and serving e107 behind one is
		// the ordinary way a site gets HTTPS.
	...
	private function probeSource()
	{
		return <<<'PHP'
<?php
// Fixture for CsrfModeMatrixCest.
