...
class DownloadTraversalCest
{
	...
	public function stillServesPluginMediaThroughRequestPhp(AcceptanceTester $I)
	{
		...
		// A Cest that installed the download plugin before this one has left
		// exactly this row, and media_url is unique, so seed it only when none did.
		$url = '{e_PLUGIN}download/images/downloads_32.png';
		$mediaId = (int) $I->grabFromDatabase('e107_core_media', 'media_id', array('media_url' => $url));
