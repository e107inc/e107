...
/**
 * e107_admin/users.php is an entry point that cannot be included from a test, so the option lists
 * behind its user_class controls are read out of the source and put through the builder the
 * controls are rendered from. A list that does not name a built-in class renders no control for
 * it, and the save that follows writes the column without it.
 *
 * What the two routes that write the column then do with a submission is asserted through the
 * running application by AdminRoutePermsCest.
 */
class usersAdminUserClassTest extends \Codeception\Test\Unit
{
	/** @var string */
	private $page;
