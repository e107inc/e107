<?php
	...
	class secure_imageTest extends \Test\Unit
	{
		...
		public function testAChallengeIsDrawnOnlyFromTheCaptchaAlphabet()
		{
			...
			self::assertSame(0, preg_match('/[^A-Z0-9]/', $alphabet),
				'the alphabet may hold no lower case, no hyphen and no underscore: '.$alphabet);
			self::assertSame(count($letters), count(array_unique($letters)),
			...
			for($i = 0; $i < 120; $i++)
			{
				...
				self::assertIsString($solution, 'the handler issued a challenge carrying no answer');
				self::assertSame(secure_image::SOLUTION_LENGTH, strlen($solution),
					'a challenge has to be the length the input and the image are sized for');
				self::assertSame('', str_replace($letters, '', $solution),
					'a challenge drew a character no visitor can read off the image: '.$solution);

				$seen += array_flip(str_split($solution));
			}

			self::assertSame(count($letters), count($seen),
		...
		public function testTheWidestChallengeFitsTheImage()
		{
			...
			{
				self::markTestSkipped('GD was built without FreeType, so no text can be measured');
			}
			...
			foreach($faces as $face => $size)
			{
				...
				self::assertNotFalse($file, 'render() picks this face at random and it is not there');
