<?php
	...
	class themeHandlerTest extends \Test\Unit
	{
		...
		public function testSetThemeConfigWritesNoPreferenceRowForAThemeDeclaringNoFields()
		{
			...
			{
				$result = $this->th->setThemeConfig();
				$rows   = e107::getDb()->createQueryBuilder()->select('e107_name')->from('core')->where('e107_name', 'theme_' . $theme)->count();
			}
			finally
			{
				$_POST = $posted;
				e107::getDb()->createQueryBuilder()->delete('core')->where('e107_name', 'theme_' . $theme)->execute();
				self::replaceThemeConfig($theme);
			}
