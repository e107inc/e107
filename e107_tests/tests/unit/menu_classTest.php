 ...
 *
 * check_class() grants everything to a CLI caller unless $_E107['phpunit'] is
 * set, so these tests set it for their own duration.
 */
class menu_classTest extends \Codeception\Test\Unit
{
	const AREA = 97;
	...
	{
		$id = e107::getDb()->insert('menus', array(
			'menu_name'     => $name,
		...
		$this->assertNotEmpty($id, 'could not write the menus row this test needs');
	}

	private function removeFixtureMenus()
	{
		e107::getDb()->delete('menus', "menu_path = '".self::FIXTURE_PATH."'");
	}
