 ...
 */
class e_file_inspector_progressTest extends \Codeception\Test\Unit
{
	const OWNER = 990001;
	...
	public function testARecordIsStampedWithTheTimeItWasMade()
	{
		...
		$stamp = e107::getDb()->retrieve('tmp', 'tmp_time', $this->ownedRow(self::OWNER));

		$this->assertGreaterThanOrEqual($before, (int) $stamp);
	}
	...
	{
		return (int) e107::getDb()->count('tmp', '(*)', $this->ownedRow($ownerId));
	}
	 ...
	 * @return string
	 */
	private function ownedRow($ownerId)
	{
		return "tmp_ip = '".e_file_inspector_progress::TMP_KEY_PREFIX.(int) $ownerId."'";
	}
}
