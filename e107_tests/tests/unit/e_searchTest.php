<?php
	...
	class e_searchTest extends \Codeception\Test\Unit
	{
		...
		protected function _after()
		{
			...
			if($this->newsSeeded)
			{
				list($output, $status) = $this->runInBootedCli(self::removeNewsItem());

				self::assertSame(0, $status,
					"The seeded news item outlived the case that put it there.\n" . implode("\n", $output));
			}
		}
		...
		private static function seedNewsItem()
		{
			...
			$php = "\$category = e107::getDb()->insert('news_category', array('category_name' => " . $category
				. ", 'category_meta_description' => '')); ";
			$php .= "e107::getDb()->insert('news', array('news_title' => " . $title . ", 'news_body' => '', "
				. "'news_extended' => '', 'news_summary' => '', 'news_meta_description' => '', "
				. "'news_thumbnail' => '', 'news_category' => \$category, 'news_datestamp' => time() - 60, "
		...
		{
			$php = "\$gone = e107::getDb()->delete('news', \"news_title = '" . self::NEWS_TITLE
				. "'\") !== false; ";
			$php .= "\$gone = e107::getDb()->delete('news_category', \"category_name = '" . self::CATEGORY_NAME
				. "'\") !== false && \$gone; ";
			$php .= "exit(\$gone ? 0 : 1); ";
