<?php
	...
	class e_searchTest extends \Test\Unit
	{
		...
		private function buildProbe()
		{
			...
			$sql->execute('INSERT INTO `' . $this->probeTable
				...
				array(
					...
					'title3'   => 'Third entry, about wibbleZwobble',
				));
		}
		...
		public function searchProbeResult($row)
		{
			...
			$res['summary'] = (string) $row['probe_summary'];
		...
		public function testEverySearchAddonSelectsEveryColumnItWeights()
		{
			...
			foreach(array_keys($this->searchAddonRows()) as $plugin)
			{
				$config = $this->searchAddon($plugin)->config();
