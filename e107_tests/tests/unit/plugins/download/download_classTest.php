 ...
 *
 * The mirror page is covered in halves, because rendering it needs the plugin's
 * tables and a row inside them, which a fresh install has neither of: the arm of
 * breadcrumb() that page asks for is driven directly here, and the order in
 * which the page fills the batch is held against the source on master, where the
 * unit base carries the helper that reads a method's own tokens.
 */
class download_classTest extends \Codeception\Test\Unit
{
	use \Test\BootedCli;
	 ...
	 * printed by the legacy database path on every page this branch serves.
	 *
