 ...
 * the filesystem.
 *
 * The release/v2.3.x CI matrix exercises PHP 5.6 → 8.2, so this file
 * sticks to syntax that parses on 5.6 (no scalar parameter or return
 * types, no short-array syntax in declarations) and uses assertions
 * present in both PHPUnit 5.7 (Codeception 4) and PHPUnit 10 (Codeception 5).
 */
class resize_handlerTest extends \Codeception\Test\Unit
{
	...
	{
		return array(
			'dollar parens' => array('out_$(touch %s).jpg'),
			'backticks'     => array('out_`touch %s`.jpg'),
			'closing quote' => array('out".$(touch %s)."after.jpg'),
		);
	}
	...
	public function testResizeImageMustNotExecuteShellMetacharactersInDestination($payloadTemplate)
	{
		...
		$source = $this->source;
		ob_start();
		$this->runWithWarningsSuppressed(function () use ($source, $destination)
		{
			resize_image($source, $destination, 400);
		});
		ob_end_clean();
	...
	public function testResizeImageWritesLiteralDestinationFilename()
	{
		...
		$source = $this->source;
		ob_start();
		$result = $this->runWithWarningsSuppressed(function () use ($source, $destination)
		{
			return resize_image($source, $destination, 400);
		});
		ob_end_clean();
	 ...
	 *
	 * Hand-rolls try/finally because PHP 5.6 doesn't support the finally
	 * keyword on closures executed through call_user_func; we restore the
	 * handler on both branches instead.
	 */
	private function runWithWarningsSuppressed($fn)
	{
	...
	/**
	 * random_bytes() polyfill bridge — PHP 5.6 lacks the native function.
	 * Composer pulls in paragonie/random_compat alongside Codeception 4,
	 * but we fall through to openssl_random_pseudo_bytes (PHP 5.3+) as a
	 * last resort so the test file can be parsed/loaded even when vendor/
	 * is in a half-installed state during CI bring-up.
	 */
