...
/**
 * Probe subclass exposing e_admin_ui's protected batch-trigger dispatch, paired with a tree
 * model that records batchUpdate() calls instead of running them.
 *
 * e_HANDLER.'admin_ui.php' must already be loaded when this file is included, so it is
 * required from the test rather than declared beside it.
 */
class AdminUiBatchProbeFixture extends e_admin_ui
{
	...
	public function setProbeFields($fields)
	{
		$this->fields = $fields;
...
/**
 * Stand-in for {@see e_admin_tree_model} recording what the batch dispatcher asks it to
 * update. Returns 0 so {@see e_admin_ui::handleListBatch()} skips the success message and the
 * list reload.
 */
