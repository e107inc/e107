 ...
 *
 * Boots e107_class.php in a subprocess through tests/_data/site_hash/boot.php, once per
 * key shape a caller can hand {@see e107::initCore()}, and reads back what it derived. The
 ...
 */
class e107SiteHashBootTest extends \Test\Unit
{
	const SHAPE_LEGACY = 'legacy';

	const SHAPE_STRIPPED = 'stripped';
	...
	{
		list($answer, $lines) = $this->boot(self::SHAPE_LEGACY);
	...
	{
		list($answer) = $this->boot(self::SHAPE_LEGACY);
	...
	public function testTheStrippedKeyShapeClass2PassesDerivesTheSameFolder()
	{
		list($answer, $lines) = $this->boot(self::SHAPE_STRIPPED);

		self::assertSame($answer['expected'], $answer['site_path'],
			'the bare keys class2.php builds from the old config format derive the same folder');
		self::assertSame($answer['known_bad_existed'], $answer['known_bad_exists']);
		self::assertCount(1, $lines, "deriving the site path raised a diagnostic:\n".implode("\n", $lines));
	}

	public function testAnEmptyServerSettingStillDerivesTheFolder()
	{
		list($answer) = $this->boot(self::SHAPE_STRIPPED, '');

		self::assertSame($answer['expected'], $answer['site_path'],
			'the key shape is told from the keys, not from what the server setting holds');
	}

	/**
	 * @param string $shape
	 * @param string|null $server replaces the server setting when given
	 * @return array the JSON answer boot.php printed, then every line it printed
	 */
	private function boot($shape, $server = null)
	{
		...
		$php .= "define('E107_BOOT_SHAPE', ".var_export($shape, true)."); ";
		if($server !== null)
		{
			$php .= "define('E107_BOOT_SERVER', ".var_export($server, true)."); ";
		}
		...
		self::assertIsArray($answer, "boot.php printed no JSON answer:\n".implode("\n", $lines));
