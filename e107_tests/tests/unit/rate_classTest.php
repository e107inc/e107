...
class rate_classTest extends \Codeception\Test\Unit
{
	...
	private function seedRate($itemid, $voters, $votes, $rating)
	{
		...
		self::assertNotEmpty(e107::getDb()->insert('rate', $data), 'Could not seed a rate row.');
	}
	...
	{
		$row = e107::getDb()->retrieve('rate', '*',
			"rate_table='".self::RATE_TABLE."' AND rate_itemid=".(int) $itemid);
