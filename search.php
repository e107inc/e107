...
class search_front extends e_shortcode
{
	...
	private function advancedFor($requested)
	{
		...
		if(count($types) > 1)
		{
			return $this->advancedTextRow(defset('LAN_SEARCH_ADVANCED_ONE_TYPE_ONLY', "Choose a single search type to see its advanced options."));
		}
			...
			foreach ($advanced as $adv_key => $adv_value) 
			{
				...
				else 
				{
					...
					if ($adv_value['type'] == 'dropdown') 
					{
						...
						$vars['SEARCH_ADV_B'] = "<select id='".$adv_key."' name='".$adv_key."' class='tbox form-control'>";
